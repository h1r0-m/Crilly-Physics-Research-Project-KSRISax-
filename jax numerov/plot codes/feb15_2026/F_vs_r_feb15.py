...
from hybrid_functions_corrected import bounded_states_solver, mu_solver_corrected, U_solver_corrected, F_solver_corrected
...
def main():
    ...
    for r in r_box_array:
        ...
        F = F_solver_corrected(e,m,d, r, r_start, N_points, mu, T_ha, Z)

        F_array_corrected.append(F)
    ...
    plt.plot(r_box_array, jnp.array(F_array_corrected) * 27.2114, linestyle = "-", marker = "x", color = "blue", label = "Hybrid (Corrected)")
    plt.xlabel("Box Radius (Ha)")
    plt.ylabel("Helmholtz Free Energy (eV)")
    plt.title(f"Helmholtz Free Energy vs r_box (T={T_ha*315775:.0f} K, N_points={N_points}, Z={Z})")
    plt.legend()
    plt.grid(True)
