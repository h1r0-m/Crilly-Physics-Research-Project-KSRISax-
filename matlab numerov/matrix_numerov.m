...
for i = 1:length(N_points_conv)
    [psi, energies, r_points] = numerov_solver(r_box, r_start, N_points_conv(i), l, n_lim);
...
for n = 1:plot_n_lim
    plot(r_points(2:end-1), psi(:,n).^2, ...
        'LineWidth', 1.5, ...
        'DisplayName', sprintf('n = %d', n));
end

xlabel('r (a_0)', 'FontSize', 15)
ylabel('|\psi_n(r)|^2', 'FontSize', 15)

title(sprintf( ...
    'Hydrogen Radial Probability Densities (l = %d, N_{points} = %d, r_{box} = %.0f)', ...
    l, N_points, r_box), ...
    'FontSize', 14)
...
filename = sprintf('probability_densities_l%d_N%d_r%.2f.png', l, N_points, r_box);

exportgraphics(gcf, filename, 'Resolution', 300);

%% functions

function [psi, energies, r_points] = numerov_solver(r_box, r_start, N_points, l, n_lim)
